...
func (h *Handlers) OrdersGet(w http.ResponseWriter, r *http.Request) {
	...
	logger.Log.Info("Get orders", zap.String("JSON", string(data)), zap.String("userID", userID))
...
func (h *Handlers) Balance(w http.ResponseWriter, r *http.Request) {
	...
	logger.Log.Info("handler balance returned", zap.String("JSON", string(dataJSON)))
	w.Header().Add("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	w.Write(dataJSON)
}
